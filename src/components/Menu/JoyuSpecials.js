...
import { JoyuSpecialData } from "../../models/mockdata";

function JoyuSpecials() {
  const navigate = useNavigate();
  const hanndleNavigate = (key) => {
    navigate("../" + path.PRODUCTDETAIL + `/${key}`, {
      state: JoyuSpecialData[key],
    });
  };
  return (
    <div className="flex flex-col justify-start items-start px-[11.7%] h-[52vw]">
      ...
      <div className="grid grid-cols-6 gap-x-[1.3vw] gap-y-[2.2vw] w-full h-fit">
        {JoyuSpecialData.map((item, index) => {
          return (
            <div
        ...
        <div className="flex flex-col items-center w-full h-full  font-shopee_regular uppercase">
          <img src={MatchaJoyu} className="w-fit h-[17.5vw]" alt=""></img>
          <p className="text-[1.15vw] h-[3.2vw]">Matcha Joyu</p>
          <p className="text-[1vw] text-primary">4.5</p>
        </div>
        <div className="flex flex-col items-center w-full h-full  font-shopee_regular uppercase">
