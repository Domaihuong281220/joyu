...
function Menu() {
    ...
    const handlegetMenu = async () => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/joyu/menu`);
    ...
    return (
      ...
      {/* <button type="button" onClick={()=>document.querySelector('#JOYU').scrollIntoView()}>test</button> */}
            ...
            {menu.map((category, index) => (
                <div key={index} 
                className={`pv:max-md:text-center,${category.products.length < 3 ? 'flex flex-col justify-start items-start px-[11.7%] pv:max-md:px-[5%] mt-[5vw] h-[30vw] pv:max-md:h-[100vw]' :category.products.length < 5 ? 'flex flex-col justify-start items-start px-[11.7%] pv:max-md:px-[5%] mt-[5vw] h-[30vw]  pv:max-md:h-[182vw]' :category.products.length < 7 ? 'flex flex-col justify-start items-start px-[11.7%] pv:max-md:px-[5%] mt-[5vw] h-[30vw]  pv:max-md:h-[265vw]' : category.products.length < 9 ? 'flex flex-col justify-start items-start px-[11.7%] pv:max-md:px-[5%] mt-[5vw] h-[55vw]  pv:max-md:h-[360vw]' :category.products.length < 11 ? 'flex flex-col justify-start items-start px-[11.7%] pv:max-md:px-[5%] mt-[5vw] h-[55vw]  pv:max-md:h-[420vw]' :category.products.length < 13 ? 'flex flex-col justify-start items-start px-[11.7%] pv:max-md:px-[5%] mt-[5vw] h-[55vw] pv:max-md:h-[530vw]' : 'flex flex-col justify-start items-start px-[11.7%] pv:max-md:px-[5%] mt-[5vw] h-[79vw]  pv:max-md:h-[565vw]'}`}
                    ...
                    <div className="grid grid-cols-6 pv:max-md:grid-cols-2 gap-x-[1.3vw] gap-y-[2.2vw] pv:max-md:gap-x-[6vw] w-full h-fit">
                        {category.products.map((product) => 
                            <div
                                ...
                                onClick={() => {
                                    handleViewProduct(product,category.category.split(" ")[0].toLowerCase());
                                }}
                            >
                                ...
                                <p className="text-[1vw] pv:max-md:text-[4vw] pv:max-md:h-fit h-[3.2vw] pv:max-md:pt-[5vw] md:max-xl:pt-[2vw]">{product.name}</p>
